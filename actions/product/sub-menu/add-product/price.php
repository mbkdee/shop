<?php
	...
	if(in_array($data->user_id, $auth->admin_list))
	{
		...
		elseif ( $constants->last_message == 'priceProAdd' ) 
		{
			...
			{
				$database->update("users", ['last_query' => 'nameProAdd'], ['id' => $data->user_id]);
			...
			else 
			{
				if (convertNumbers($data->text)>=0 && is_numeric(convertNumbers($data->text)))
				{
					...
					$telegram->sendMessage([
					'chat_id' => $data->user_id,
					'text' => "✅ لطفا کمترین مقدار مجاز که کاربر می تواند سفارش دهد را وارد نمایید.",
					'reply_markup' => $keyboard->go_back_one_step()
					]);
				} 
				else 
				{
					...
					$telegram->sendMessage([
					'chat_id' => $data->user_id,
					'text' => "⚠️ قیمت را به صورت یک عدد صحیح بزرگتر از صفر وارد نمایید."."\n"."✅ لطفا قیمت محصول مورد نظر را به تومان وارد کنید.",
					'reply_markup' => $keyboard->go_back_one_step()
					]);
				}
			}
